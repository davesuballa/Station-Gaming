  ...
  $('.play').on('click', function(){
    ...
    $('.logo').fadeOut(250, function(){
      ...
      for(i = 0; i < shu.length; i++){
        var imgIndex = shu[i] % images.length;
        var imageUrl = images[imgIndex];
        $('<div class="card" style="width:'+cardSize+'%;height:'+cardSize+'%;">'+
            '<div class="flipper"><div class="f"></div><div class="b"><img src="' + imageUrl + '" alt="Card Image"></div></div>'+
          '</div>').appendTo('#g');
      }
      ...
      $('#g .card').on({
        'mousedown' : function(){
          ...
          var $this = $(this).addClass('active').find('.b img').attr('src');

          if($('#g').find('.card.active').length > 1){
            setTimeout(function(){
              var activeCards = $('#g .active .b img[src="'+$this+'"]');
              if (activeCards.length > 1) {
                activeCards.parents('.card').toggleClass('active card found').empty(); 
